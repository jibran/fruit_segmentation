"""Swin Transformer V2 baseline classifier for fruit segmentation comparison.

Uses the same pretrained Swin-V2 backbone as ``SwinUNet`` but replaces the
U-Net decoder with a global-average-pool + linear head.  This gives a fair
baseline: identical backbone weights, identical number of output classes,
but no pixel-level segmentation capability.

The baseline operates in *image-level classification* mode — it predicts a
single class label per image (the dominant fruit class) rather than a
per-pixel mask.

Usage::

    python models/swin_baseline.py  # quick self-test

    # In a training / eval script:
    from models.swin_baseline import SwinBaseline
    model = SwinBaseline(num_classes=17, size="tiny",
                         pretrained=True, input_size=512)
"""
...
# ── Backbone registry (mirrors swin_unet.py) ──────────────────────────────────
...
# Final-stage (stage 4) channel widths — used to size the linear head
_SWIN_STAGE4_CHANNELS: dict[str, int] = {
    "tiny": 768,
    "small": 768,
    "base": 1024,
}
...
class SwinBaseline(nn.Module):
    """Swin-V2 backbone with a global-average-pool classification head.

    Intended as a baseline to compare against ``SwinUNet``.  The backbone
    is identical; the difference is that this model produces one class
    prediction per image rather than one per pixel.

    Architecture::

        Input (B, 3, H, W)
            ↓
        Swin-V2 encoder  (pretrained, 4 stages + patch merging)
            ↓
    ...
    Args:
        ...
        pretrained: Whether to load ImageNet pretrained weights.
            Defaults to ``True``.
        input_size: Square spatial resolution fed to the model.  Passed
            to timm so it can interpolate position bias tables when the
            inference resolution differs from the pretraining resolution
            (256).  Defaults to 512.
        drop_rate: Dropout probability applied before the linear head.
            Defaults to 0.2.

    Example:
        >>> model = SwinBaseline(num_classes=17, size="tiny", input_size=512)
    ...
    def __init__(
        ...
        # Full backbone with global pooling and no classifier head
        backbone_kwargs = dict(
            pretrained=pretrained,
            num_classes=0,  # remove timm's own classifier head
            global_pool="avg",  # global average pool to (B, C)
            img_size=input_size,
        )
        try:
            self.backbone = timm.create_model(_SWIN_TIMM_NAMES[size], **backbone_kwargs)
        except (RuntimeError, ValueError):
            self.backbone = timm.create_model(
                _SWIN_TIMM_FALLBACKS[size], **backbone_kwargs
            )

        in_features = _SWIN_STAGE4_CHANNELS[size]
    ...
    # ------------------------------------------------------------------
    # Backbone freeze / unfreeze helpers (mirror SwinUNet API)
    # ------------------------------------------------------------------
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            x: Batch of RGB images, shape ``(B, 3, H, W)``.
                ``H`` and ``W`` should equal ``input_size``.
...
if __name__ == "__main__":
    print("Swin-V2 Baseline — self-test (pretrained=False, tiny, input_size=512)")
    model = SwinBaseline(num_classes=17, size="tiny", pretrained=False, input_size=512)
    model.eval()
